...
Last updated: 2026-08-14 (Step 17 complete; Step 18 next)

## Current milestone

Phase C, Step 17: Hints + optional earned boosters.
Steps 01-16 are complete. Step 17 ships a deterministic
legal-move hint ranker (`SugartrailHints`) and a minimal
approved booster set (`SugartrailBooster`) with one entry:
SWAP_RETRY. Boosters use a two-phase request/cancel/confirm
flow; cancel does not consume inventory and confirm is
atomic. The replay engine supports booster actions and the
session roundtrips the booster pack through snapshot.
SWAP_RETRY snapshots the pre-swap board and restores from it,
refunding the move and removing the swap from the action
log so retries cannot be applied to the same swap twice.
26 new tests (hints, boosters, boosters_integration) bring
the total to 254 (253 passing + 1 risky/pending for an
unreachable deadlock precondition). The engine version is
...
| Run unit tests | `bash tools/test.sh` | 2026-08-14 — exit 0, 253/253 passing (board 10, rng 9, rules 13, version 3, resolution 14, replay 12, gameplay 7, session 16, levels_validation 10, levels_curated 13, specials_data 10, specials_activation 13, specials_integration 17, combos 18, combos_integration 7, blockers 16, blockers_layers 6, blockers_integration 6, objectives 9, tokens 8, objectives_integration 8, hints 8, boosters 11, boosters_integration 11) |
